# #!/usr/bin/env python3
# """
# Enhanced DLStreamer Benchmark Monitor
# Consumes Kafka messages from DLStreamer inference pipeline and tracks detailed performance metrics
# including per-core CPU usage, GPU utilization, thermal monitoring, and frame drop analysis
# """
...
#             'start_time': None,
#             'last_update': None,
#             'fps_history': deque(maxlen=60),  # Keep 60 seconds of FPS data
...
#         """Get CPU and memory stats for DLStreamer container"""
#         try:
...
#                     mem_used = float(values[1]) if values[1] != '[N/A]' else 0
#                     mem_total = float(values[2]) if values[2] != '[N/A]' else 0
#                     temp = float(values[3]) if values[3] != '[N/A]' else 0
#                     power = float(values[4]) if len(values) > 4 and values[4] != '[N/A]' else 0
#                     clock_gpu = float(values[5]) if len(values) > 5 and values[5] != '[N/A]' else 0
...
#             recent_fps = list(metrics['fps_history'])[-10:]  # Last 10 measurements
#             fps_variance = sum((fps - sum(recent_fps)/len(recent_fps))**2 for fps in recent_fps) / len(recent_fps)
#             fps_stability = 1 / (1 + fps_variance) if fps_variance > 0 else 1  # Higher = more stable
#         else:
#             fps_variance = 0
#             fps_stability = 1
        
#         # Calculate processing efficiency
#         target_fps = 30  # Source FPS
...
#             if not camera_id:
#                 # Try to extract from video path or other fields
#                 if 'video' in data:
...
#             if not camera_id:
#                 print(f"Warning: Could not extract camera ID from message")
#                 return
...
#                 # Calculate processing time from timestamp
#                 try:
#                     if isinstance(data['timestamp'], (int, float)):
#                         # Timestamp is already a number (milliseconds or seconds)
#                         msg_time = data['timestamp']
#                         if msg_time > 1e12:  # Milliseconds
#                             msg_time = msg_time / 1000
#                     else:
#                         # String timestamp
#                         msg_time = datetime.fromisoformat(data['timestamp'].replace('Z', '+00:00')).timestamp()
                    
#                     processing_time = (current_time - msg_time) * 1000  # Convert to ms
#                     if 0 < processing_time < 10000:  # Reasonable range (0-10 seconds)
...
#         """Enhanced metrics logging with detailed CPU, GPU, and timing stats"""
#         while True:
#             try:
#                 current_time = time.time()

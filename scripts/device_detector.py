...
#!/usr/bin/env python3
"""Intel Arc A770 Device Detector"""

import subprocess
import sys

def detect_arc_gpu():
    """Detect Intel Arc A770 GPU"""
    try:
        # Check lspci for Arc GPU
        result = subprocess.run(['lspci', '-nn'], capture_output=True, text=True, timeout=5)
        if result.returncode == 0:
            for line in result.stdout.split('\n'):
                if 'VGA' in line or 'Display' in line:
                    if '5690' in line or 'Arc' in line.lower():
                        return True, 'Intel Arc A770 detected'
        
        # Check VAAPI for AV1 support (Arc-specific)
        result = subprocess.run(['vainfo'], capture_output=True, text=True, timeout=5, 
                              env={'LIBVA_DEVICE': '/dev/dri/renderD129'})
        if result.returncode == 0 and 'VAProfileAV1Profile0' in result.stdout:
            return True, 'Intel Arc GPU detected (AV1 support)'
        
        return False, 'No Arc GPU detected'
    except:
...
if __name__ == "__main__":
    ...
    detected, info = detect_arc_gpu()
    
    if args.verbose:
        print("="*70)
        print("INTEL ARC A770 DETECTION")
        print("="*70)
        print(f"\n{'✅' if detected else '❌'} {info}")
        print(f"\nDevice: {'GPU.1' if detected else 'CPU'}")
        print(f"DRI Device: {'/dev/dri/renderD129' if detected else 'N/A'}")
        print("="*70)
    
    print(f"DEVICE={'GPU.1' if detected else 'CPU'}")
    print(f"INFO={info}")
    print(f"STATUS={'OPTIMAL' if detected else 'FALLBACK'}")
